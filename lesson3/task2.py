...
def get_list_info(lst):
    min_elem = min(lst)
    max_elem = max(lst)
    sum_list = round(sum(lst), 2)  # сумма всех элементов, округленная до 2 знаков после запятой
    average = round((sum(lst))/len(lst), 2)  # ср.ар. = сумма всех / на кол-во элементов, которое получаем через len
