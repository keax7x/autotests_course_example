...
def repeats(our_str):
    """
    функция, которая примает строку, смотрит сколько раз встречается буква в строке и записывает новую строку
    с результатом
    :param our_str: строка, которую принимаем на вход, состоящая из букв
    :return: new_str: новая строка вида "буква_цифра", где строки и цифра- сколько раз эту букву в строке встретили
    """
    tmp_dict = {}  # временный словарь, куда записываем сколько раз встретили букву
    new_str = ''
    #  в процессе итерации по строке
    for bukva in our_str:
        if tmp_dict.get(bukva):  # проверяем есть ли в словаре буква
            tmp_dict[bukva] += 1  # увеличиваем занчение ключа буквы в словаре
            new_str += f"{bukva}_{tmp_dict[bukva]}"  # записываем в строку букву и значение ключа по букве из словаря
        else:
            tmp_dict[bukva] = 1  # если нет то значение ключа 1
            new_str += f"{bukva}_{1}"  # в строку записываем букву и значение 1
    return new_str
