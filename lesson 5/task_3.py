...
def everything_for_your_cat(cats_data):
    """
    Функция для оптимизации хранения данных, которая для каждого владельца показывает данные о всех его животных в 1
    строке, причем каждый след владелец с животными на новой строке
    :param cats_data: список кортежей, где каждый состоит из Кличка котика, Возраст котика, Имя покупателя, Фамилия пок.
    :return: возвращаемая одна строка, но с условием, что каждый владелец с новой
    """
    test_dict = {}
    for k in cats_data:
        owner = f"{k[2]} {k[3]}:"  # записали имя и фамилию владельца в строку
        pet = f'{k[0]}, {k[1]}'  # записали кличку и возвраст питомца в строку
        if not test_dict.get(owner):   # если не встретили владельца в словаре
            test_dict[owner] = pet   # то значением ключа владельца записываем строку с животным
        else:
            test_dict[owner] += f"; {pet}"  # то значением ключа владельца добавляем сумму строк из питомцев
    our_str = ''
    for owner in test_dict:
        our_str += f"{owner} {test_dict[owner]}\n"  # добавлем в 1 строку, и выводим каждого влад с животными с новой
    return our_str
